...
async fn main() {
    ...
    let upbit_exchange: Box<dyn Exchange> = Box::new(Upbit::new(upbit_api_key, upbit_secret).unwrap());
    let binance_exchange: Box<dyn Exchange> = Box::new(Binance::new(bithumb_api_key, bithumb_secret).unwrap());
